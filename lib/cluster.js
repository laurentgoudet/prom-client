...
class AggregatorRegistry extends Registry {
	...
	clusterMetrics(callback) {
		...
		return new Promise((resolve, reject) => {
			...
			for (const id in require('cluster').workers) {
				// If the worker exits abruptly, it may still be in the workers
				// list but not able to communicate.
				if (require('cluster').workers[id].isConnected()) {
					require('cluster').workers[id].send(message);
					request.pending++;
				}
			}
...
process.on('message', message => {
	if (message.type === GET_METRICS_REQ && require('cluster').isWorker) {
		process.send({
